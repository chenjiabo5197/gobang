...
SDLButton::SDLButton(const Config& config, const std::string button_name)
{
    ...
    this->button_position.x = config.Read(button_name+"_x", 0);
    ...
    this->button_name, this->button_position.x, this->button_position.y, this->button_width, this->button_height, this->button_multiple, this->button_resource_path);
}
...
bool SDLButton::loadResource(SDL_Window * gWindow, SDL_Renderer* gRenderer)
{
   //Load data
    ...
    //Map colors
    Uint32 colorKey = SDL_MapRGBA(SDL_GetWindowSurface(gWindow)->format, 255, 255, 255, 0xFF);   //取出按键周围的颜色，用下面的值将其设置为透明色
...
bool SDLButton::buttonRender(SDL_Renderer* gRenderer)
{
